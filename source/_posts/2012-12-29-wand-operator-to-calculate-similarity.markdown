...
上面提到公式不对称，主要就是差在一个文档长度上，如果求$Score(d_a, d_b)$则分母是$\|d_b\|$，如果求$Score(d_b, d_a)$分母为$\|d_a\|$。如果定义Score为再除以一个$\|d_a\|$使得公式对称，这样也没有问题，但是通常情况下我们需要的是与$d_a$最相似的n个文档，相似度计算中是否除以$\|d_a\|$并不会影响相似文档的排名。
